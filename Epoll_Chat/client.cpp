...
int main(int argc, char *argv[]) {
    ...
    serverAddr.sin_port = htons(SERVER_PORT); // htons 把主机字节转化成网络字节的函数
    serverAddr.sin_addr.s_addr = inet_addr(SERVER_IP); // inet_addr 将字符串形式的IP地址转换为按网络字节顺序的整型值
    ...
    if (pid < 0) {
        ...
        while (isClientwork) {
            ...
            // bzero() 会将内存块（字符串）的前n个字节清零; s为内存（字符串）指针，n 为需要清零的字节数。
            fgets(message, BUF_SIZE, stdin);
            // 客户输出exit,退出 strncasecmp 比较字符串的前n个字符
            if (strncasecmp(message, EXIT, strlen(EXIT)) == 0) {
                isClientwork = false ;
            } else {    // 子进程将信息写入管道 ; write 就是将缓冲区的数据写出到标准输出上
